...
//qianshi
static bool tchdbiternextintotime(TCHDB *hdb, char**ppKey, int* keylen, time_t* pTime);
...
/* Get the next extensible objects of the iterator of a hash database object. */
bool tchdbiternext4(TCHDB *hdb, char **kxkey, time_t *kxtime){
    assert(hdb && kxtime);
    if(!HDBLOCKMETHOD(hdb, true)) return false;
    if(hdb->fd < 0 || hdb->iter < 1){
        tchdbsetecode(hdb, TCEINVALID, __FILE__, __LINE__, __func__);
    ...
    char *pkeyTmp = NULL;
    int keytlen;
    bool rv = tchdbiternextintotime(hdb, &pkeyTmp,&keytlen,  kxtime);
    if (rv){
        *kxkey = malloc(keytlen+1);
        memcpy(*kxkey, pkeyTmp, keytlen);
        (*kxkey)[keytlen] = 0;
        free(pkeyTmp);
    }
...
bool tchdbiternextintotime(TCHDB *hdb, char** ppKey, int *keylen, time_t* pTime){
    assert(hdb && pTime);
    TCHREC rec;
    char rbuf[HDBIOBUFSIZ];
    while(hdb->iter < hdb->fsiz){
        ...
        if(rec.magic == HDBMAGICREC){
            /*if(rec.kbuf){
                keylen = rec.ksiz;
               // TCMEMDUP(pTime, &rec.kbuf[keylen], sizeof(time_t));
                *pTime = *(time_t*)&rec.kbuf[keylen];
                return true;
            }*/
            if(!tchdbreadrecbody(hdb, &rec)) return false;
            *ppKey = rec.bbuf;
            *keylen = rec.ksiz;
            *pTime = *(time_t*)&rec.bbuf[rec.ksiz];
            return true;
        }
    }
    tchdbsetecode(hdb, TCENOREC, __FILE__, __LINE__, __func__);
...
/* qianshi */
bool tchdbwrite(TCHDB *hdb, off_t off, const void *buf, size_t size){
    return tchdbseekwrite(hdb, off, buf, size);
}
